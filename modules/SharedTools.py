...
class Installer:
    ...
    def check_install(self):
        ...
        try:
            exit_code = subprocess.call(
                [self.executable_path, "--return-exit-code", "999"],
    ...
    def install(self):
        if self.check_install():
            ...
            console_log(
                "The program is already installed!!!", OK, silent_mode=SILENT_MODE
            )
            console_log(
                f"Location: {self.executable_path}", WARN, silent_mode=SILENT_MODE
            )
            return True
        if sys.platform.startswith("win") or sys.platform == "darwin":
            if I_AM_EXECUTABLE:
                try:
                    ...
                    logging.info(
                        f"The program was successfully installed on the path: {self.executable_path}"
                    )
                    console_log(
                        f"The program was successfully installed on the path: {self.executable_path}",
                        OK,
                    ...
                    logging.error(
                        "No write access, try running the program with elevated permissions!!!"
                    )
                    console_log(
                        "No write access, try running the program with elevated permissions!!!",
                        ERROR,
                    ...
                    logging.error(
                        "Installation is pointless from under an installed executable file!!!"
                    )
                    console_log(
                        "Installation is pointless from under an installed executable file!!!",
                        ERROR,
                ...
                console_log(
                    "Installation from source is not possible!!!!",
                    ERROR,
                    silent_mode=SILENT_MODE,
                )
            return False
...
class ChromeProxyExtensionManager:
    ...
    def create_extension(
        scheme: string, host: string, port: int, username="", password=""
    ):
        ...
        if username != "" or password != "":
            with open(tempdir.joinpath("background.js"), "a") as f:
                f.write(
                    ChromeProxyExtensionManager.BACKGROUND_AUTO_AUTH
                    % (username, password)
                )
...
from .WebDriverInstaller import (
    APPLE_SAFARI,
    GOOGLE_CHROME,
    MICROSOFT_EDGE,
    MOZILLA_FIREFOX,
...
def initSeleniumWebDriver(
    ...
    console_log(
        f"{colorama.Fore.LIGHTMAGENTA_EX}-- Browsers Initializer --{colorama.Fore.RESET}\n",
        silent_mode=SILENT_MODE,
    )
    if os.name == "posix":  # For Linux
        if sys.platform.startswith("linux"):
            ...
            console_log(
                f"Initializing {browser_name} for Linux", INFO, silent_mode=SILENT_MODE
            )
        elif sys.platform == "darwin":
            logging.info(f"Initializing {browser_name} for macOS")
            console_log(
                f"Initializing {browser_name} for macOS", INFO, silent_mode=SILENT_MODE
            )
    elif os.name == "nt":
        logging.info(f"Initializing {browser_name} for Windows")
        console_log(
            f"Initializing {browser_name} for Windows", INFO, silent_mode=SILENT_MODE
        )
    driver_options = None
    driver = None
    if browser_name == GOOGLE_CHROME:
        ...
        try:
            ...
            if (
                traceback.format_exc().find("only supports") != -1
                ...
                if "new_chrome.exe" in os.listdir(browser_path[:-10]):
                    logging.info(
                        "Downloaded Google Chrome update is detected! Using new chrome executable file!"
                    )
                    console_log(
                        "Downloaded Google Chrome update is detected! Using new chrome executable file!",
                        INFO,
        ...
        try:
            ...
            logging.critical("EXC_INFO:", exc_info=True)
            if (
                traceback.format_exc().find("--user-data-dir") != -1
        ...
        try:
            ...
            driver = Safari(
                options=driver_options,
                service=SafariService(executable_path=webdriver_path),
            )
        except Exception as e:
            logging.critical("EXC_INFO:", exc_info=True)
            if traceback.format_exc().find("Allow Remote Automation") != -1:
                console_log(
                    traceback.format_exc().split("Message: ")[-1].strip(), ERROR
                )
            else:
                raise e
    return driver
...
def parseToken(
    ...
    if email_obj.class_name == "custom":
        while True:
            activated_href = input(
                f"\n[  {colorama.Fore.YELLOW}INPT{colorama.Fore.RESET}  ] {colorama.Fore.CYAN}Enter the link to activate your account, it will come to the email address you provide: {colorama.Fore.RESET}"
            ).strip()
            logging.info(
                f"[  INPT  ] Enter the link to activate your account, it will come to the email address you provide: {activated_href}"
            )
            if activated_href != "":
                if eset_business:
    ...
    for _ in range(max_iter):
        if email_obj.class_name == "1secmail":
            ...
            for mail in inbox:
                ...
                if (
                    ...
                    or mail_from.find("ESET HOME") != -1
                    or mail_subject.find("ESET PROTECT Hub") != -1
                ):
                    email_obj.open_mail(mail_id)
                    if email_obj.class_name in [
                        "mailticking",
                        "incognitomail",
                        "emailfake",
                    ]:
                    ...
                    try:
                        if email_obj.class_name == "mailticking":
                            driver.switch_to.frame(
                                driver.find_element("id", "email-iframe")
                            )
                        if eset_business:
                            activated_href = driver.find_element(
                                "xpath",
                                "//a[starts-with(@href, 'https://protecthub.eset.com')]",
                            ).get_attribute("href")
                        else:
                            activated_href = driver.find_element(
                                "xpath",
        ...
        time.sleep(delay)
    raise RuntimeError(
        "Token retrieval error, try again later or change the Email API!!!"
    )
...
def parseEPHKey(email_obj, driver=None, delay=DEFAULT_DELAY, max_iter=DEFAULT_MAX_ITER):
    for _ in range(max_iter):
        ...
        if email_obj.class_name in ["developermail", "inboxes"]:
            ...
            inbox = []
            # Retry inbox parsing on network error
            for _ in range(3):
                try:
                    inbox = email_obj.parse_inbox()
                    if inbox:  # valid list returned
                        break
                except Exception as e:
                    print(f"[  WARN  ] Inbox parse error: {e}, retrying...")
                    time.sleep(3)
            ...
            if inbox:
                for mail in inbox:
                    ...
                    if (
                        "thank you for purchasing" in subject_lower
                        or "how to get started" in subject_lower
                    ):
                        try:
                            email_obj.open_mail(mail_id)
                            time.sleep(3)
                            if email_obj.class_name == "mailticking":
                                license_data = driver.find_element(
                                    "id", "email-iframe"
                                ).srcdoc
                            else:
        ...
        if license_data is not None:
            ...
            try:
                license_id = re.search(
                    r"[A-Z0-9]{3}-[A-Z0-9]{3}-[A-Z0-9]{3}", license_data
                ).group()
            except:
                license_id = "N/A"  # New email might not have License ID

            # Try multiple date format patterns
            license_out_date = None
            try:
                # Try old format: dd.mm.yyyy or dd/mm/yyyy or dd-mm-yyyy
                matches = re.findall(r"\d\d[./-]\d\d[./-]\d\d\d\d", license_data)
                if matches:
                    license_out_date = matches[-1]
            except:
                pass

            if not license_out_date:
                try:
                    # Try new format: "Month day, year" or "day Month year"
                    # Look for text like "January 1, 2027" or "1 January 2027"
                    month_pattern = r"(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+\d{4}"
                    match = re.search(month_pattern, license_data, re.IGNORECASE)
                    if match:
                        license_out_date = match.group()
                except:
                    pass

            if not license_out_date:
                try:
                    # Fallback: look for any 4-digit year near "valid" text
                    valid_pattern = r"valid[^0-9]{0,50}(\d{4})"
                    match = re.search(valid_pattern, license_data, re.IGNORECASE)
                    if match:
                        license_out_date = match.group(1)
                except:
                    pass
...
def parseVPNCodes(
    email_obj, driver=None, delay=DEFAULT_DELAY, max_iter=DEFAULT_MAX_ITER
):
    for _ in range(max_iter):
        data = None
        try:
                    ...
                    if mail_subject.find("VPN - Setup instructions") != -1:
                        ...
                        if email_obj.class_name == "mailticking":
                            ...
                            try:
                                data = str(
                                    driver.execute_script(
                                        f'return {GET_EBCN}("email_body")[0].innerHTML'
                                    )
                                )
                            except:
                                pass
                            ...
                            try:
                                data = str(
                                    driver.execute_script(
                                        f'return {GET_EBCN}("MuiBox-root joy-1v8x7dw")[0].innerHTML'
                                    )
                                )
                            except:
                                pass
        ...
        time.sleep(delay)
    raise RuntimeError(
        "VPN Codes retrieval error, try again later or change the Email API!!!"
    )
